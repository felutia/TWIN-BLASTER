...
namespace game
{
    ...
    /// バレットクラス
    /// </summary>
    public abstract class IEnemyBullet
		: IObject
	{
		/// <summary>
		/// コンストラクタ
		/// </summary>
		public IEnemyBullet( COLOR_ID color , bool reflec, Vector2 pos, float direction, float speed, int power )
			: base()
		{
			// 色設定
			m_Color				= color;
		...
		protected void CheckArea( bool reflec )
		{
			...
			if( m_Sprite.Position.X < 0.0f - 32.0f )
			{
				m_ReflecFlag = false;
				m_IsActive = false;
			}
			...
			if( m_Sprite.Position.Y > 544.0f - 32.0f )
			{
				if( reflec == true )
				{
					m_Sprite.Position.Y = 544.0f - 32.0f;
